...
import gaugeLib.hansolo.medusa.events.AlarmEvent;
import gaugeLib.hansolo.medusa.events.AlarmEventListener;
import gaugeLib.hansolo.medusa.events.TimeEvent;
import gaugeLib.hansolo.medusa.events.TimeEvent.TimeEventType;
import gaugeLib.hansolo.medusa.events.TimeEventListener;
import gaugeLib.hansolo.medusa.events.UpdateEvent;
import gaugeLib.hansolo.medusa.events.UpdateEvent.EventType;
import gaugeLib.hansolo.medusa.events.UpdateEventListener;
import gaugeLib.hansolo.medusa.skins.ClockSkin;
import gaugeLib.hansolo.medusa.skins.DBClockSkin;
import gaugeLib.hansolo.medusa.skins.DesignClockSkin;
import gaugeLib.hansolo.medusa.skins.DigitalClockSkin;
import gaugeLib.hansolo.medusa.skins.FatClockSkin;
import gaugeLib.hansolo.medusa.skins.IndustrialClockSkin;
import gaugeLib.hansolo.medusa.skins.LcdClockSkin;
import gaugeLib.hansolo.medusa.skins.MinimalClockSkin;
import gaugeLib.hansolo.medusa.skins.PearClockSkin;
import gaugeLib.hansolo.medusa.skins.PlainClockSkin;
import gaugeLib.hansolo.medusa.skins.RoundLcdClockSkin;
import gaugeLib.hansolo.medusa.skins.SlimClockSkin;
import gaugeLib.hansolo.medusa.skins.TextClockSkin;
import gaugeLib.hansolo.medusa.skins.TileClockSkin;
import gaugeLib.hansolo.medusa.tools.Helper;
import gaugeLib.hansolo.medusa.tools.TimeSectionComparator;
...
public class Clock extends Control {
      ...
      // Alarm events
      private final List<UpdateEventListener> listenerList = new CopyOnWriteArrayList<>();
      private final List<AlarmEventListener> alarmListenerList = new CopyOnWriteArrayList<>();
      private final List<TimeEventListener> timeEventListenerList = new CopyOnWriteArrayList<>();
      ...
      public long getTimeMs() {
            return time.get().toInstant().toEpochMilli();
      }
      ...
      public BooleanProperty checkSectionsForValueProperty() {
            if (null == checkSectionsForValue) {
                  checkSectionsForValue = new SimpleBooleanProperty(Clock.this, "checkSectionsForValue", _checkSectionsForValue);
            }
      ...
      public BooleanProperty checkAreasForValueProperty() {
            if (null == checkAreasForValue) {
                  checkAreasForValue = new SimpleBooleanProperty(Clock.this, "checkAreasForValue", _checkAreasForValue);
            }
       ...
       * @param SECTIONS
       */
      public void setSections(final TimeSection... SECTIONS) {
       ...
       * @param AREAS
       */
      public void setAreas(final TimeSection... AREAS) {
      ...
      public BooleanProperty discreteHoursProperty() {
            if (null == discreteHours) {
                  discreteHours = new SimpleBooleanProperty(Clock.this, "discreteHours", _discreteHours);
            }
      ...
      public boolean isAutoNightMode() {
            return null == autoNightMode ? _autoNightMode : autoNightMode.get();
      }
      ...
      public BooleanProperty autoNightModeProperty() {
            if (null == autoNightMode) {
                  autoNightMode = new SimpleBooleanProperty(Clock.this, "autoNightMode", _autoNightMode);
            }
      ...
      public BooleanProperty animatedProperty() {
            if (null == animated) {
                  animated = new SimpleBooleanProperty(Clock.this, "animated", _animated);
            }
            return animated;
      }
      ...
      public void setAnimationDuration(final long ANIMATION_DURATION) {
            animationDuration = Helper.clamp(10, 20000, ANIMATION_DURATION);
      }
            ...
            Platform.runLater(() -> {
                  ...
                  if (getCheckSectionsForValue()) {
                        ...
                        for (int i = 0; i < listSize; i++) {
                              sections.get(i).checkForValue(LocalTime.from(now));
                        }
                  }
                  if (getCheckAreasForValue()) {
                        int listSize = areas.size();
                        for (int i = 0; i < listSize; i++) {
                              areas.get(i).checkForValue(LocalTime.from(now));
                        }
                  }
                  ...
                  if (oldTime.getSecond() != now.getSecond())
                        fireTimeEvent(new TimeEvent(Clock.this, now, TimeEventType.SECOND));
                  if (oldTime.getMinute() != now.getMinute())
                        fireTimeEvent(new TimeEvent(Clock.this, now, TimeEventType.MINUTE));
                  if (oldTime.getHour() != now.getHour())
                        fireTimeEvent(new TimeEvent(Clock.this, now, TimeEventType.HOUR));
            });
      }
      ...
      protected Skin createDefaultSkin() {
            switch (skinType) {
                  ...
                  case PEAR:
                        return new PearClockSkin(Clock.this);
                  case PLAIN:
                        return new PlainClockSkin(Clock.this);
                  case DB:
                        return new DBClockSkin(Clock.this);
                  case FAT:
                        return new FatClockSkin(Clock.this);
                  case ROUND_LCD:
                        return new RoundLcdClockSkin(Clock.this);
                  case SLIM:
                        return new SlimClockSkin(Clock.this);
                  case MINIMAL:
                        return new MinimalClockSkin(Clock.this);
                  case DIGITAL:
                        return new DigitalClockSkin(Clock.this);
                  case TEXT:
                        return new TextClockSkin(Clock.this);
                  case DESIGN:
                        return new DesignClockSkin(Clock.this);
                  case INDUSTRIAL:
                        return new IndustrialClockSkin(Clock.this);
                  case TILE:
                        return new TileClockSkin(Clock.this);
                  case CLOCK:
                  default:
                        return new ClockSkin(Clock.this);
            }
      ...
      public void addTimeEventListener(final TimeEventListener LISTENER) {
            if (!timeEventListenerList.contains(LISTENER)) timeEventListenerList.add(LISTENER);
      }

      public void removeTimeEventListener(final TimeEventListener LISTENER) {
            timeEventListenerList.remove(LISTENER);
      }

      public void fireTimeEvent(final TimeEvent EVENT) {
            int listSize = timeEventListenerList.size();
            for (int i = 0; i < listSize; i++) {
                  timeEventListenerList.get(i).onTimeEvent(EVENT);
            }
      }
}
