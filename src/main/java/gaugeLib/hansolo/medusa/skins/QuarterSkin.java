...
import gaugeLib.hansolo.medusa.Gauge.LedType;
...
import gaugeLib.hansolo.medusa.Needle;
import gaugeLib.hansolo.medusa.Section;
import gaugeLib.hansolo.medusa.TickLabelLocation;
import gaugeLib.hansolo.medusa.TickLabelOrientation;
import gaugeLib.hansolo.medusa.TickMarkType;
...
public class QuarterSkin extends GaugeSkinBase {
      ...
      protected void handleEvents(final String EVENT_TYPE) {
            ...
            if ("FINISHED".equals(EVENT_TYPE)) {
                  ...
                  // gaugeLib.hansolo.medusa.Check sections for value and fire section events
                  ...
                  // gaugeLib.hansolo.medusa.Check areas for value and fire section events
                  ...
                  maxValue = gauge.getMaxValue();
                  angleStep = ANGLE_RANGE / gauge.getRange();
                  if (gauge.getValue() < minValue) {
                        oldValue = minValue;
                  }
                  if (gauge.getValue() > maxValue) {
                        oldValue = maxValue;
                  }
                  resize();
                  redraw();
      ...
      private double getStartAngle() {
            ...
            switch (knobPosition) {
                  case BOTTOM_LEFT:
                        return ScaleDirection.CLOCKWISE == scaleDirection ? 180 : 90;
                  case TOP_LEFT:
                        return ScaleDirection.CLOCKWISE == scaleDirection ? 90 : 0;
                  case TOP_RIGHT:
                        return ScaleDirection.CLOCKWISE == scaleDirection ? 0 : 270;
                  case BOTTOM_RIGHT:
                  default:
      ...
      private void resizeText() {
            ...
            if (titleText.getLayoutBounds().getWidth() > maxWidth) {
                  Helper.adjustTextSize(titleText, maxWidth, size * 0.06);
            }
            ...
            if (unitText.getLayoutBounds().getWidth() > maxWidth) {
                  Helper.adjustTextSize(unitText, maxWidth, size * 0.05);
            }
            ...
            if (subTitleText.getLayoutBounds().getWidth() > maxWidth) {
                  Helper.adjustTextSize(subTitleText, maxWidth, size * 0.05);
            }
