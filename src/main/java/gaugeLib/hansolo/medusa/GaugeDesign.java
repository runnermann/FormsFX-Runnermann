...
import gaugeLib.hansolo.medusa.tools.ConicalGradient;
...
public enum GaugeDesign {
      NONE(0) {
            @Override
            public Border getBorder(final double SIZE) {
                  return Border.EMPTY;
            }
      },
      METAL(0.08333333) {
      ...
      public enum GaugeBackground {
            DARK_GRAY() {
                  @Override
                  public Paint getPaint(final double X1, final double Y1, final double X2, final double Y2) {
                        Stop[] stops = {
