...
public class Sleep {
      ...
      /**
       * When sleep is detected, we save. However, in reality, this method is called
       * once the OS has killed its sleep state.
            ...
            Observable.merge(sceneEventPublishable, windowEventPublishable)
                ...
                .subscribe(e -> {
                      if (!isSent) {
                            ...
                            if (ReadFlash.getInstance() != null || CreateFlash.getInstance() != null) {
                                  FlashCardOps.getInstance().unsafeSaveFlashList(); //saveFListToFile();
                            }
                      }
                });
      }
