     ...
     protected ArrayList<FlashCardMM> getListFmFile(String pathName) {
        ...
        try (ObjectInputStream input = new ObjectInputStream(new BufferedInputStream(new FileInputStream(pathName)))) {
            ...
            LOGGER.warn("\t****** IO Exception in FlashCard.connectBinaryIn() *******");
              // Set errorMsg in FlashMonkeyMain and add to the not compatible
              // list
              FlashCardOps.getInstance().addToNotCompatibleList(fileName);
              FlashMonkeyMain.deckNotCompatiblePopup();
        } catch (Exception e) {
              // Set errorMsg in FlashMonkeyMain and add to the not compatible
              // list
              FlashCardOps.getInstance().addToNotCompatibleList(fileName);
...
//        }
    ...
    /**
     * Expects all data to be of String. Using writeUTF. If
    ...
    public boolean saveImage(String imgFileName, Image image, String mime, char type) {
        ...
        try {
            ...
            LOGGER.warn("ERROR: image not save. Illegal Argument exception. A parameter was null: " + e.getMessage());
        } catch (Exception e) {
